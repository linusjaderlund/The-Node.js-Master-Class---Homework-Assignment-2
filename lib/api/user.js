...
user.get = (data, callback) => {
  const mail = val.def(data.query.mail, is.mail, false);
  if (!mail) {
    return callback(400);
  }

  store.read(collection, mail, (err, obj) => {
...
user.put = (data, callback) => {
  ...
  store.read(collection, mail, (err, obj) => {
    ...
    store.update(collection, mail, merge, (err) => {
      if (err) {
        return callback(500);
      }
...
user.delete = (data, callback) => {
  ...
  store.delete(collection, mail, (err) => {
    if (err) {
